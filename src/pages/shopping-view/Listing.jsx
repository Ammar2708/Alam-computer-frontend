...
const categorySeoMap = {
  Laptop: {
    ...
    description:
      "Shop laptops in Sharjah at Alam Computer. Browse laptops for home, office, business, and everyday use with local support and current availability.",
  },
  Lcd: {
    title: "Computer Monitors in Sharjah",
    description:
      "Shop computer monitors in Sharjah at Alam Computer. Browse monitors for home, office, gaming, and business use.",
  },
  Printer: {
    title: "Printers in Sharjah",
    description:
      "Shop printers in Sharjah at Alam Computer. Browse new and refurbished printers for home, office, and business use with local support.",
  },
  Ink: {
    title: "Printer Ink in Sharjah",
    description:
      "Shop printer ink in Sharjah at Alam Computer. Find ink supplies for home and office printers with local availability and support.",
  },
  SSD: {
    title: "SSD Drives in Sharjah",
    description:
      "Shop SSD drives in Sharjah at Alam Computer. Browse storage upgrades for laptops and desktop computers with local support.",
  },
  Network: {
    title: "Networking Products in Sharjah",
    description:
      "Shop networking products in Sharjah at Alam Computer, including equipment and accessories for home and business networks.",
  },
  "All In One": {
    title: "All-in-One Computers in Sharjah",
    description:
      "Shop all-in-one computers in Sharjah at Alam Computer. Browse compact systems for home, office, and business use.",
  },
  Towner: {
    title: "Printer Toners in Sharjah",
    description:
      "Shop printer toners in Sharjah at Alam Computer. Find toner supplies for home and office printers with local stock and support.",
  },
  accessories: {
    title: "Computer Accessories in Sharjah",
    description:
      "Shop computer accessories in Sharjah at Alam Computer. Browse cables, peripherals, components, and everyday computer accessories.",
  },
};
...
const ShoppingListing = ({ setOpenCartSheet }) => {
  ...
  const categorySeo = activeCategory
    ? categorySeoMap[activeCategory]
    : null;

  const seoTitle =
    categorySlug && categorySeo
      ? categorySeo.title
      : searchQuery
      ? `Search Results for "${searchQuery}"`
  ...
  const handleFilter = (
    ...
    if (!cpyFilters[sectionId]) {
      cpyFilters[sectionId] = [
        currentOption,
      ];
    } else {
      const optionIndex =
        cpyFilters[sectionId].indexOf(
          currentOption
        );
    ...
    // A category slug represents a fixed category.
    // Move to the general listing when the user
    // changes sidebar filters so their selection
    // is not overridden.
    if (categorySlug) {
      navigate(
        `/shop/listing${
  ...
  const addProductToCartForUser =
    async (
      getCurrentProductId,
      loggedInUser = user
    ) => {
      const activeUserId =
        getCartOwnerId(loggedInUser);
  ...
  useEffect(() => {
    const cleanedParams =
      removeUnavailableCategoriesFromSearchParams(
        searchParamsString
      );
  ...
  return (
    <>
      ...
      <div className="flex min-h-screen flex-col gap-4 bg-gray-50 p-3 sm:p-4 lg:flex-row lg:gap-6">
        ...
        <div className="min-w-0 flex-1 rounded-xl border bg-white">
          ...
          {categorySlug === "printers" && (
            <div className="mx-4 mt-4 rounded-2xl border border-red-100 bg-red-50 p-5 sm:mx-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <Link
                  ...
                  className="inline-flex shrink-0 items-center justify-center rounded-xl bg-red-600 px-5 py-3 text-sm font-black text-white transition hover:bg-red-700"
                >
                  View Printer Repair
                  Service
                </Link>
      ...
      >
        <SheetContent
          side="left"
          className="w-[90vw] overflow-y-auto bg-white p-0 sm:max-w-sm"
        >
